...
public class ModelCMMRBlaze extends ModelBase
{
    ModelRenderer HTop;
    ModelRenderer Hair;
    ModelRenderer Head;
    ...
    private ModelRenderer[] BlazeRods;
    private int textureWidthbyped;
    private int textureHeightbyped;
    
    public ModelCMMRBlaze(final float modelSize, final float offset) {
        this.BlazeRods = new ModelRenderer[12];
        ...
        (this.BHair = new ModelRenderer((ModelBase)this, 30, 14)).addBox(-4.0f, 1.0f, 2.333333f, 8, 12, 1);
        ...
        this.setRotation(this.BustTop, -0.7888888f, 0.0f, 0.0f);
        ...
        (this.BustBottom = new ModelRenderer((ModelBase)this, 48, 60)).addBox(-3.0f, 0.0f, -0.01f, 6, 2, 2);
        ...
        this.setRotation(this.BustBottom, -0.4537856f, 0.0f, 0.0f);
        (this.Body2 = new ModelRenderer((ModelBase)this, 0, 55)).addBox(-3.0f, 6.0f, -1.5f, 6, 4, 3);
        this.Body2.setRotationPoint(0.0f, 0.0f, 0.0f);
        this.Body2.setTextureSize(this.textureWidth, this.textureHeight);
        this.Body2.mirror = true;
        this.setRotation(this.Body2, 0.0f, 0.0f, 0.0f);
        (this.Skirt1 = new ModelRenderer((ModelBase)this, 0, 35)).addBox(-3.5f, 0.0f, -1.5f, 7, 3, 3);
        ...
        if (!YarrCuteMobModelsRemake.BlazeHideBlazeRods) {
            for (int i = 0; i < this.BlazeRods.length; ++i) {
                if (!YarrCuteMobModelsRemake.humanMobsModels) {
                    (this.BlazeRods[i] = new ModelRenderer((ModelBase)this, 26, 0)).addBox(0.0f, 0.0f, 0.0f, 1, 4, 1);
                }
                if (YarrCuteMobModelsRemake.humanMobsModels) {
                    (this.BlazeRods[i] = new ModelRenderer((ModelBase)this, 56, 16)).addBox(0.0f, 0.0f, 0.0f, 2, 8, 2);
                }
            }
        }
    }
    ...
    public void setRotationAngles(final float par1, final float par2, final float par3, final float par4, final float par5, final float par6, final Entity par7Entity) {
        ...
        if (!YarrCuteMobModelsRemake.BlazeHideBlazeRods) {
            ...
            for (int i = 0; i < 4; ++i) {
                this.BlazeRods[i].rotationPointY = -2.0f + MathHelper.cos((i * 2 + par3) * 0.25f);
                this.BlazeRods[i].rotationPointX = MathHelper.cos(f6) * 9.0f;
                this.BlazeRods[i].rotationPointZ = MathHelper.sin(f6) * 9.0f;
                ++f6;
            }
            f6 = 0.7853982f + par3 * 3.1415927f * 0.03f;
            for (int i = 4; i < 8; ++i) {
                this.BlazeRods[i].rotationPointY = 2.0f + MathHelper.cos((i * 2 + par3) * 0.25f);
                this.BlazeRods[i].rotationPointX = MathHelper.cos(f6) * 7.0f;
                this.BlazeRods[i].rotationPointZ = MathHelper.sin(f6) * 7.0f;
                ++f6;
            }
            f6 = 0.47123894f + par3 * 3.1415927f * -0.05f;
            for (int i = 8; i < 12; ++i) {
                this.BlazeRods[i].rotationPointY = 11.0f + MathHelper.cos((i * 1.5f + par3) * 0.5f);
                this.BlazeRods[i].rotationPointX = MathHelper.cos(f6) * 5.0f;
                this.BlazeRods[i].rotationPointZ = MathHelper.sin(f6) * 5.0f;
                ++f6;
            }
        }
        if (this.isRiding) {
