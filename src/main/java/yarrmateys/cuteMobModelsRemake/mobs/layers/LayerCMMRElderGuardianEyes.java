...
public class LayerCMMRElderGuardianEyes implements LayerRenderer
{
    ...
    private final RenderCMMRElderGuardian Renderer;
    
    public LayerCMMRElderGuardianEyes(final RenderCMMRElderGuardian par1) {
        this.Renderer = par1;
    }
    
    public void doRenderLayerEX(final EntityElderGuardian entitylivingbaseIn, final float limbSwing, final float limbSwingAmount, final float partialTicks, final float ageInTicks, final float netHeadYaw, final float headPitch, final float scale) {
        if (!YarrCuteMobModelsRemake.humanMobsModels) {
            this.Renderer.bindTexture(LayerCMMRElderGuardianEyes.texture1b);
        }
        else {
            this.Renderer.bindTexture(LayerCMMRElderGuardianEyes.texture1bBl);
        }
    ...
    public void doRenderLayer(final EntityLivingBase entitylivingbaseIn, final float limbSwing, final float limbSwingAmount, final float partialTicks, final float ageInTicks, final float netHeadYaw, final float headPitch, final float scale) {
        this.doRenderLayerEX((EntityElderGuardian)entitylivingbaseIn, limbSwing, limbSwingAmount, partialTicks, ageInTicks, netHeadYaw, headPitch, scale);
    }
